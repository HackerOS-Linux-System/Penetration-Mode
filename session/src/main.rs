...
use penetration_mode_ipc::protocol::{SdeCall, SdeEvent, SdeResult, SdeWindowInfo, Subscription};
...
struct Cli {
    ...
    /// Force plain-app mode's bare-tty fallback (`hackeros-comp --extern-other`)
...
/// Name hackeros-comp was published under — see `install.hl`
/// (`$PREFIX/bin/hackeros-comp` in the `hackeros-comp` repository). Resolved via
/// `PATH` rather than hardcoded to that one path so a locally-built
/// `target/release/hackeros-comp` on `PATH` during development is picked up
/// too.
const HACKEROS_COMP_BIN: &str = "hackeros-comp";
...
/// The shell window's `app_id` as hackeros-comp reports it over `ListWindows`/
...
/// here too so [`run_via_hackeros_comp`] can stay generic over
/// `penetration_mode_ipc::protocol` directly instead of depending on the
/// crate's higher-level, `penetration-mode`-specific convenience wrappers
/// for just this one string.
const SESSION_EXTERN_NAME: &str = "penetration-mode";

/// `--extern-other`'s name — a throwaway, non-curated hackeros-comp instance
...
/// to `hackeros-comp --extern-penetration-mode`, exactly like a
/// `wayland-sessions/*.desktop` entry launching it would (see
/// `desktop/penetration-mode-session.desktop`). hackeros-comp itself (not
...
fn run_session_mode() -> Result<()> {
    let mut compositor = spawn_hackeros_comp(SESSION_EXTERN_NAME)
        .context("nie udało się uruchomić hackeros-comp --extern-penetration-mode")?;

    if let Err(e) = wait_until_up(SESSION_EXTERN_NAME, &mut compositor) {
        let _ = compositor.kill();
        return Err(e).context("hackeros-comp --extern-penetration-mode nie odpowiedziało na czas");
    }

    run_via_hackeros_comp(SESSION_EXTERN_NAME, &mut compositor)
}
...
fn run_app_mode(force_other: bool) -> Result<()> {
    ...
    if already_nested && !force_other {
        // Zwykła apka na już działającym pulpicie — bez hackeros-comp/IPC w
    ...
    let mut compositor =
        spawn_hackeros_comp(OTHER_EXTERN_NAME).context("nie udało się uruchomić hackeros-comp --extern-other")?;

    if let Err(e) = wait_until_up(OTHER_EXTERN_NAME, &mut compositor) {
        let _ = compositor.kill();
        return Err(e).context("hackeros-comp --extern-other nie odpowiedziało na czas");
    }

    run_via_hackeros_comp(OTHER_EXTERN_NAME, &mut compositor)
}

/// Shared tail end of both modes once a `hackeros-comp --extern-<extern_name>`
...
/// `Shutdown` hackeros-comp, wait for that process to exit.
fn run_via_hackeros_comp(extern_name: &str, compositor: &mut Child) -> Result<()> {
    penetration_mode_ipc::protocol::call(extern_name, SdeCall::LaunchApp { command: SHELL_BIN.to_string(), args: vec![] }, CALL_TIMEOUT)
        .context("hackeros-comp odrzuciło LaunchApp dla powłoki Penetration Mode")?;
    ...
    let _ = penetration_mode_ipc::protocol::call(extern_name, SdeCall::Shutdown, CALL_TIMEOUT);
    let status = compositor.wait().context("błąd oczekiwania na hackeros-comp")?;
    if !status.success() {
        bail!("hackeros-comp zakończyło się z błędem: {status}");
    }
    Ok(())
}

fn spawn_hackeros_comp(extern_name: &str) -> Result<Child> {
    Command::new(HACKEROS_COMP_BIN)
        .arg(format!("--extern-{extern_name}"))
        .stdin(Stdio::null())
        .spawn()
        .with_context(|| format!("nie znaleziono '{HACKEROS_COMP_BIN}' na PATH"))
}
...
/// Czeka aż `hackeros-comp --extern-<extern_name>` zacznie odpowiadać na
/// `Ping` (`penetration_mode_ipc::protocol::is_running`) — realny sygnał gotowości, nie samo
/// istnienie pliku socketu (co nie gwarantuje że listener już obsługuje
/// requesty). Sprawdza po drodze, czy sam proces hackeros-comp jeszcze żyje,
...
fn wait_until_up(extern_name: &str, compositor: &mut Child) -> Result<()> {
    ...
    loop {
        if penetration_mode_ipc::protocol::is_running(extern_name) {
            return Ok(());
        }
        if let Some(status) = compositor.try_wait()? {
            bail!("hackeros-comp zakończyło się przedwcześnie ({status}) zanim odpowiedziało na Ping");
        }
        if Instant::now() >= deadline {
            bail!("timeout ({READY_TIMEOUT:?}) czekając na hackeros-comp --extern-{extern_name}");
        }
...
/// [`run_via_hackeros_comp`] — na wypadek gdyby okno zmapowało się w tym samym
...
/// żadnego "stanu początkowego" przy otwarciu (patrz `penetration_mode_ipc::protocol`'s moduł
...
/// w `penetration_mode_ipc::protocol`) — [`spawn_event_reader`] czyta ją na osobnym wątku i
...
/// nigdy się nie pojawiło i hackeros-comp nigdy więcej nic nie wysłało.
fn wait_for_shell_window(extern_name: &str, compositor: &mut Child) -> Result<()> {
    ...
    loop {
        ...
        match rx.recv_timeout(remaining) {
            ...
            Ok(Err(e)) => {
                if let Some(status) = compositor.try_wait()? {
                    bail!("hackeros-comp zakończyło się nieoczekiwanie ({status}) zanim okno powłoki się pojawiło");
                }
                bail!("subskrypcja penetration-mode-ipc padła zanim okno powłoki się pojawiło: {e}");
            }
            ...
            Err(mpsc::RecvTimeoutError::Disconnected) => {
                bail!("wątek subskrypcji penetration-mode-ipc padł nieoczekiwanie zanim okno powłoki się pojawiło")
            }
        }
    }
}
...
/// [`wait_for_shell_window`]. Zamknięcie procesu hackeros-comp zamyka też ten
...
fn wait_for_shell_window_gone(extern_name: &str, compositor: &mut Child) -> Result<()> {
    ...
    loop {
        match rx.recv() {
            ...
            Ok(Ok(_)) => continue,
            Ok(Err(_)) | Err(_) => {
                // Zerwane połączenie zwykle znaczy, że hackeros-comp właśnie
                // się zamknęło (patrz `SdeEvent::CompositorShuttingDown`'s
                // doc w penetration_mode_ipc::protocol: dziś jeszcze nie wysyłane explicite,
                // zamknięcie socketu jest tym sygnałem) — a skoro
                // hackeros-comp już nie działa, to i powłoka jako jego klient
...
fn shell_window_now(extern_name: &str) -> Result<bool> {
    match penetration_mode_ipc::protocol::call(extern_name, SdeCall::ListWindows, CALL_TIMEOUT)? {
...
fn open_subscription(extern_name: &str, compositor: &mut Child) -> Result<Subscription> {
    if let Some(status) = compositor.try_wait()? {
        bail!("hackeros-comp już nie działa ({status}), nie ma do czego subskrybować");
    }
    penetration_mode_ipc::protocol::subscribe(extern_name).context("nie udało się otworzyć subskrypcji penetration-mode-ipc")
}
